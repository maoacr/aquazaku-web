...
export function BuscadorDeCliente({
  ...
  return (
    <div className="aq-etiqueta-campo">
      ...
      {/* Este div posiciona, y por eso NO lleva clase del sistema: las del
          sistema declaran `position: relative` sin capa y le ganarían. */}
      <div className="relative">
        ...
        <input
          ...
          onKeyDown={alTeclear}
          className="aq-campo aq-cifra pl-9"
        />
        ...
        {abierto ? (
          <div className="absolute top-full right-0 left-0 z-10 mt-1">
            <ul
              ...
              role="listbox"
              aria-label={etiqueta}
              className="aq-menu-vidrio max-h-72 overflow-y-auto p-1"
            >
              {resultados.map((cliente, i) => (
                <li
                  ...
                  className={`flex min-h-11 cursor-pointer items-center justify-between gap-3 rounded-sm px-3 ${
                    i === resaltado ? 'bg-accion-suave' : ''
                  }`}
                >
                  <span className="min-w-0 truncate text-[15px] text-principal">
                    {cliente.nombre}
                  </span>
                  <span className="aq-cifra shrink-0 text-[13px] text-tenue">
                    {cliente.documento}
                  </span>
                </li>
              ))}
              ...
              {resultados.length === 0 ? (
                <li className="flex min-h-11 items-center px-3 text-[13px] text-tenue">
