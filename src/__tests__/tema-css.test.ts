...
function propiedadesDe(css: string, selector: string): string[] {
  const bloque = new RegExp(`${selector}\\s*\\{([\\s\\S]*?)\\n\\s*\\}`).exec(css)
  if (!bloque) throw new Error(`no se encontró el bloque ${selector}`)

  return [...bloque[1]!.matchAll(/^\s*(--[\w-]+):/gm)].map((m) => m[1]!).sort()
}

describe('el modo sistema pinta lo mismo que el oscuro explícito', () => {
  const explicito = [
    ...propiedadesDe(tokens, '\\[data-tema="oscuro"\\]'),
    ...propiedadesDe(globales, "\\[data-tema='oscuro'\\]"),
  ].sort()

  const porSistema = propiedadesDe(globales, ':root:not\\(\\[data-tema\\]\\)')

  it('declaran exactamente las mismas propiedades', () => {
    expect(porSistema).toEqual(explicito)
  })

  it('no es una lista vacía que pase por casualidad', () => {
    expect(explicito.length).toBeGreaterThan(20)
  })

  it('incluye las superficies: son las que se ven primero si falta algo', () => {
    expect(porSistema).toContain('--aq-superficie-fondo')
    expect(porSistema).toContain('--aq-texto-principal')
  })
})
